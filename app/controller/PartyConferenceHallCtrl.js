App.controller('PartyConferenceHallCtrl', ['$scope', '$state', '$rootScope', '$http', 'ngDialog', 'PagerExtends', 'layerAlert', 'serverUrls', 'PcService', '$q',
	function($scope, $state, $rootScope, $http, ngDialog, PagerExtends, layerAlert, serverUrls, PcService, $q) {
		...
		$scope.toggleItem = function(x) {
			...
			$scope.listBusyPromise = $http({
				...
				method: "get",
				url: serverUrls.councilchamberopenstate + "?id=" + x.Id + "&state=" + state
		...
		$scope.fetchData = function() {
			...
			promises.then(function() {
				PagerExtends.regListSpecifyPage($scope, {
					apiUrl: serverUrls.councilchamberlist,
		...
		$scope.detailItem = function(x) {
			...
			$state.go("app.partyConferenceHallDetail", {
